        ...
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    let user = new User(topicResult[0]['USERNAME'], topicResult[0]['EMAIL'], topicResult[0]['PROFESSION'], topicResult[0]['NAME'], topicResult[0]['GENDER'],new Date(topicResult[0]['BIRTHDAY']), topicResult[0]['USER_ID']);
                    ...
                    for (let i = 0; i < postsResult.length; i++) {
                        let user = new User(postsResult[i]['USERNAME'], postsResult[i]['EMAIL'], postsResult[i]['PROFESSION'], postsResult[i]['NAME'], postsResult[i]['GENDER'], new Date(postsResult[i]['BIRTHDAY']), postsResult[i]['USER_ID']);
